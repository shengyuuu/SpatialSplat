...
class DPTOutputAdapter_fix(DPTOutputAdapter):
    ...
    def init(self, dim_tokens_enc=768):
        ...
        #         nn.BatchNorm2d(self.feature_dim) if self.use_bn else nn.Identity(),
        #         nn.ReLU(),
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], depths, imgs, image_size=None, conf=None):
        ...
        # Output head
        out = self.head(path_1)
        # opacity = self.opacity_head(path_1)
        # out = torch.cat([opacity,out], dim=1)
        # active = self.active_head(path_1)
        # out = torch.cat([out,active], dim=1)
        
        if not self.language_scale:
            return out, None
        else:
            path_1_down = F.interpolate(path_1.clone(), size=(H//self.language_scale, W//self.language_scale), mode='bilinear', align_corners=True)
            out_down = self.down_head(path_1_down)
            # out_down[:,-1,:,:] = 
            return out, out_down
class PixelwiseTaskWithDPT(nn.Module):
    ...
    def __init__(self, *, n_cls_token=0, hooks_idx=None, dim_tokens=None,
                 output_width_ratio=1, num_channels=1, language_scale = 0., postprocess=None, depth_mode=None, conf_mode=None, **kwargs):
...
def create_gs_dpt_head(net, has_conf=False, out_nchan=3, language_scale = 0., postprocess_func=postprocess):
    """
