...
from .spatial_attention import SpatialTransformer
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if use_spatial_transformer: # sy: not used
            assert context_dim is not None, 'Fool!! You forgot to include the dimension of your cross-attention conditioning...'

        if context_dim is not None: # sy: not used
            assert use_spatial_transformer, 'Fool!! You forgot to use the spatial transformer for your cross-attention conditioning...'
            from omegaconf.listconfig import ListConfig
            if type(context_dim) == ListConfig:
                context_dim = list(context_dim)

        if num_heads_upsample == -1: # sy: num_heads_upsample = -1, num_heads == -1, num_head_channels=32
        ...
        self.image_size = image_size # sy: =none
        self.in_channels = in_channels # sy: =32
        self.model_channels = model_channels# sy: =32
        self.out_channels = out_channels# sy: =32
        self.num_res_blocks = num_res_blocks# sy: =1
        self.attention_resolutions = attention_resolutions # sy: =[16]
        self.dropout = dropout # sy: =0
        self.channel_mult = channel_mult #sy: =[1,1,1,1,1]
        self.conv_resample = conv_resample # sy: =True
        self.num_classes = num_classes # sy: =None
        self.use_checkpoint = use_checkpoint # sy: =False
        self.dtype = th.float16 if use_fp16 else th.float32
        self.num_heads = num_heads # sy: =-1
        self.num_head_channels = num_head_channels # sy: =32
        self.num_heads_upsample = num_heads_upsample # sy: =-1
        self.predict_codebook_ids = n_embed is not None

        self.middle_block_attn = middle_block_attn # sy: =False

        self.middle_block_no_identity = middle_block_no_identity # sy: =False
        ...
        self.cross_attn_condition = cross_attn_condition # sy: =False
        ...
        for level, mult in enumerate(channel_mult):
            # sy: just a list of resblocks, channels are remian unchanged
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions: # sy: only the last level has attention
                    if num_head_channels == -1:
            ...
            if level != len(channel_mult) - 1:
                # sy: not the last level, downsample
                out_ch = ch
        ...
        if self.middle_block_attn: # sy: False
            # pass
            ...
            if self.middle_block_no_identity: # sy:false
                # pass
                self.middle_block = nn.Sequential(
                ResBlock(
                    ch,
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                self._feature_size += ch
        self.return_idx = [0, 2, 4, 6, 9] #! NOTE: MODIFY THIS FOR YOUR OWN MODEL    
