...
def matrix_to_quaternion(matrices, eps: float = 1e-8):
    """
    Convert rotation matrices (B, 3, 3) to quaternions (B, 4).
    Returns quaternions in (x, y, z, w) order.
    """
    if matrices.ndim == 2:
        matrices = matrices.unsqueeze(0)
    B = matrices.shape[0]
    m = matrices
    qw = torch.sqrt(torch.clamp(1 + m[:,0,0] + m[:,1,1] + m[:,2,2], min=eps)) / 2
    qx = torch.sqrt(torch.clamp(1 + m[:,0,0] - m[:,1,1] - m[:,2,2], min=eps)) / 2
    qy = torch.sqrt(torch.clamp(1 - m[:,0,0] + m[:,1,1] - m[:,2,2], min=eps)) / 2
    qz = torch.sqrt(torch.clamp(1 - m[:,0,0] - m[:,1,1] + m[:,2,2], min=eps)) / 2

    q = torch.stack([qx, qy, qz, qw], dim=-1)

    return q # Ensure the last element is positive
