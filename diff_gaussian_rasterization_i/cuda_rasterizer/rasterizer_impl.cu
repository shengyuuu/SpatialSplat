	...
	bool include_feature)
{
	...
	if (radii[idx] > 0)
	{
		...
		if (include_feature)
			getRect_CLIP(points_xy[idx], radii[idx], rect_min, rect_max, grid);
		else
...
int CudaRasterizer::Rasterizer::forward(
	...
	const float* instance_feature_precomp, // 增加了参数
	...
	bool include_instance_feature)
{
	...
	dim3 tile_grid;
	dim3 block;
	// sy: add include_feature start
	if (include_feature)
	{	
		tile_grid = dim3((width + BLOCK_X_CLIP - 1) / BLOCK_X_CLIP, (height + BLOCK_Y_CLIP - 1) / BLOCK_Y_CLIP, 1);
		block = dim3(BLOCK_X_CLIP, BLOCK_Y_CLIP, 1);
	}
	else
	{
	...
	// std::cout << "tile_grid: " << tile_grid.x << " " << tile_grid.y << std::endl;
	...
	CHECK_CUDA(FORWARD::render(
		...
		include_feature,
		include_instance_feature), debug) // 增加了参数
	...
	bool include_instance_feature)
{
	...
	CHECK_CUDA(BACKWARD::render(
		...
		include_feature,
		include_instance_feature), debug)
